...
void core1_main(void)
{
    ...
    while (TRUE)
    {
        ...
        guimai_board_tx_core_task();
        mag_cal_logger_task(now_ms);
        nav_path_worker_task(now_ms);
        data_logger_worker_task(now_ms);
        data_logger_camera_snapshot_worker_task(now_ms);
        data_logger_camera_record_worker_task(now_ms);
        mag_cal_logger_worker_task(now_ms);
        system_delay_ms(guimai_voice_is_recording() ? 1 : 5);
