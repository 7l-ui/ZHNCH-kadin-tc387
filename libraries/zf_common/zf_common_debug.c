...
static volatile uint8       zf_debug_uart_suppressed = 0;
...
{
    if (zf_debug_uart_suppressed == 0U)
    {
...
void debug_uart_output_suppress(uint8 suppress)
{
    zf_debug_uart_suppressed = (suppress != 0U) ? 1U : 0U;
    __dsync();
}

uint8 debug_uart_output_is_suppressed(void)
{
    __dsync();
    return zf_debug_uart_suppressed;
}
