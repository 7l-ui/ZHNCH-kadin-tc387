...
#define WIFI_UART_DIAG_ENABLE   ( 1 )                                           // Print AT response when an ACK wait fails
...
static uint8 wifi_uart_wait_ack (char *wait_buffer, uint32 timeout)
{
    ...
    char receiver_buffer[128] = {0};
    char receiver_tail_buffer[128] = {0};
    uint32 receiver_len = 0;
    uint32 receiver_tail_len = 0;
#if WIFI_UART_DIAG_ENABLE
    uint8 diag_buffer[WIFI_UART_BUFFER_SIZE + 1] = {0};
    uint32 diag_len = 0;
#endif
    ...
    {
        system_delay_ms(1);
        // 同时检查 FIFO 头部和尾部，兼容 AT+GMR 这类 OK 出现在长响应末尾的命令
        memset(receiver_buffer, 0, sizeof(receiver_buffer));
        memset(receiver_tail_buffer, 0, sizeof(receiver_tail_buffer));
        receiver_len = (uint32)(sizeof(receiver_buffer) - 1U);
        receiver_tail_len = (uint32)(sizeof(receiver_tail_buffer) - 1U);
        fifo_read_buffer(&wifi_uart_fifo, (uint8 *)receiver_buffer, &receiver_len, FIFO_READ_ONLY);
        fifo_read_tail_buffer(&wifi_uart_fifo, (uint8 *)receiver_tail_buffer, &receiver_tail_len, FIFO_READ_ONLY);
        receiver_buffer[receiver_len] = '\0';
        receiver_tail_buffer[receiver_tail_len] = '\0';
        if(strstr(receiver_buffer, wait_buffer) || strstr(receiver_tail_buffer, wait_buffer))
        {
            return_state = 0;
            break;
        }
        else if(strstr(receiver_buffer, "ERROR") || strstr(receiver_buffer, "busy") ||
                strstr(receiver_tail_buffer, "ERROR") || strstr(receiver_tail_buffer, "busy"))
        {
    ...
    if(return_state)
    {
        diag_len = (uint32)(sizeof(diag_buffer) - 1U);
        fifo_read_buffer(&wifi_uart_fifo, diag_buffer, &diag_len, FIFO_READ_ONLY);
        if(diag_len >= sizeof(diag_buffer))
        {
            diag_len = (uint32)(sizeof(diag_buffer) - 1U);
        }
        diag_buffer[diag_len] = '\0';
        debug_send_buffer((const uint8 *)"[WIFI_UART_ACK] wait=", strlen("[WIFI_UART_ACK] wait="));
        debug_send_buffer((const uint8 *)wait_buffer, strlen(wait_buffer));
        debug_send_buffer((const uint8 *)" fail rx=\"", strlen(" fail rx=\""));
        debug_send_buffer((const uint8 *)diag_buffer, strlen((char *)diag_buffer));
...
{
    while(uart_query_byte(WIFI_UART_INDEX, &wifi_uart_data))                    // 读取串口硬件 FIFO 中已有的数据
    {
...
uint8 wifi_uart_init (char *wifi_ssid, char *pass_word, wifi_uart_mode_enum wifi_mode)
{
    ...
    do
    {
        ...
        uart_config_ret = wifi_uart_uart_config_set(uart_baud, "8", "1", "0", "0"); // 调用接口重设模块的工作串口参数
        ...
        if(uart_config_ret)
        {
            ...
            uart_write_string(WIFI_UART_INDEX, "AT\r\n");
            if(wifi_uart_wait_ack("OK", WAIT_TIME_OUT))
            {
...
#if WIFI_UART_DIAG_ENABLE
            debug_send_buffer((const uint8 *)"[WIFI_UART] baud switch confirmed\r\n",
                              strlen("[WIFI_UART] baud switch confirmed\r\n"));
#endif
        }
