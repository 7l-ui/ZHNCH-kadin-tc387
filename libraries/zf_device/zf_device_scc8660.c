...
static volatile uint32 scc8660_vsync_count = 0U;
static volatile uint32 scc8660_dma_frame_count = 0U;
static volatile uint32 scc8660_dma_lost_count = 0U;
static volatile uint8 scc8660_init_attempts = 0U;
static volatile uint8 scc8660_init_result = 0xFFU;

static uint32 scc8660_uncached_shared_address(uint32 address)
{
...
static volatile uint8 *scc8660_frame_flag_shared(void)
{
    return (volatile uint8 *)scc8660_uncached_shared_address(
        (uint32)&scc8660_finish_flag);
}

uint8 scc8660_frame_is_ready(void)
{
    __dsync();
    return *scc8660_frame_flag_shared();
}

void scc8660_frame_release(void)
{
    *scc8660_frame_flag_shared() = 0U;
    __dsync();
}

const uint16 *scc8660_get_frame_buffer(void)
{
    return (const uint16 *)scc8660_uncached_shared_address(
        (uint32)&scc8660_image[0][0]);
}

static void scc8660_frame_set_ready(uint8 ready)
{
    *scc8660_frame_flag_shared() = ready;
    __dsync();
}
...
/* Camera exposure and white-balance defaults are selected in the header. */
...
static void scc8660_vsync_handler(void)
{
    ...
    /* Keep the completed single-buffer frame stable until the consumer releases it. */
...
static void scc8660_dma_handler(void)
{
    ...
    {
        scc8660_dma_lost_count++;
        scc8660_frame_set_ready(0U);
        dma_disable(SCC8660_DMA_CH);
        IfxDma_clearChannelTransactionRequestLost(&MODULE_DMA, SCC8660_DMA_CH);
    ...
    else
    {
        ...
        if(scc8660_dma_int_num >= scc8660_link_list_num)
        {
            ...
            scc8660_dma_int_num = 0;
            scc8660_lost_flag   = 0;
            scc8660_frame_set_ready(1U);
            scc8660_dma_frame_count++;
            dma_disable(SCC8660_DMA_CH);
        }
    }
}

uint32 scc8660_get_vsync_count(void)
{
    return scc8660_vsync_count;
}

uint32 scc8660_get_dma_frame_count(void)
{
    return scc8660_dma_frame_count;
}

uint32 scc8660_get_dma_lost_count(void)
{
    return scc8660_dma_lost_count;
}

uint8 scc8660_get_init_attempts(void)
{
    return scc8660_init_attempts;
}

uint8 scc8660_get_init_result(void)
{
    return scc8660_init_result;
}
...
uint8 scc8660_init (void)
{
    ...
    scc8660_vsync_count = 0U;
    scc8660_dma_frame_count = 0U;
    scc8660_dma_lost_count = 0U;
...
uint8 scc8660_init_sccb(void)
{
    ...
    scc8660_dma_frame_count = 0U;
    scc8660_dma_lost_count = 0U;
    scc8660_init_attempts = 0U;
    scc8660_init_result = 0xFFU;
    scc8660_frame_set_ready(0U);
    scc8660_lost_flag = 1U;
    scc8660_dma_init_flag = 1U;
    ...
    set_camera_type(CAMERA_COLOR,
                    scc8660_vsync_handler,
                    scc8660_dma_handler,
                    NULL);
    scc8660_type = SCC8660_SCCB;

    for (attempt = 0U;
         attempt < SCC8660_SCCB_INIT_RETRY_COUNT;
         attempt++)
    {
        scc8660_init_attempts = (uint8)(attempt + 1U);
        soft_iic_init(&iic,
                      0,
                      SCC8660_COF_IIC_DELAY,
                      SCC8660_COF_IIC_SCL,
                      SCC8660_COF_IIC_SDA);
        result = scc8660_set_config_sccb(&iic,
                                          scc8660_set_confing_buffer);
        if (result == 0U)
        {
            break;
        }
        system_delay_ms(SCC8660_SCCB_INIT_RETRY_DELAY_MS);
    }

    if (result == 0U)
    {
        scc8660_link_list_num = camera_init(SCC8660_DATA_ADD,
                                             (uint8 *)scc8660_image[0],
                                             SCC8660_IMAGE_SIZE);
    }
    scc8660_init_result = result;
    return result;
}
