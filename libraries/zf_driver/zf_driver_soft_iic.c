...
#ifndef SOFT_IIC_INTERNAL_PULLUP_COMPAT
#define SOFT_IIC_INTERNAL_PULLUP_COMPAT  (1)
#endif
...
// ????     ????
//-------------------------------------------------------------------------------------------------------------------
static void soft_iic_sda_drive_low (soft_iic_info_struct *soft_iic_obj)
{
#if SOFT_IIC_INTERNAL_PULLUP_COMPAT
    gpio_set_dir(soft_iic_obj->sda_pin, GPO, GPO_OPEN_DTAIN);
#endif
    soft_iic_gpio_low_sda();
}
...
// ????     ????
//-------------------------------------------------------------------------------------------------------------------
static void soft_iic_sda_release_high (soft_iic_info_struct *soft_iic_obj)
{
#if SOFT_IIC_INTERNAL_PULLUP_COMPAT
    gpio_set_dir(soft_iic_obj->sda_pin, GPI, GPI_PULL_UP);
#else
    soft_iic_gpio_high_sda();
#endif
}
...
// ????     ????
//-------------------------------------------------------------------------------------------------------------------
static void soft_iic_sda_prepare_output (soft_iic_info_struct *soft_iic_obj)
{
#if SOFT_IIC_INTERNAL_PULLUP_COMPAT
    gpio_set_dir(soft_iic_obj->sda_pin, GPO, GPO_OPEN_DTAIN);
#endif
...
static void soft_iic_start (soft_iic_info_struct *soft_iic_obj)
{
    ...
    soft_iic_sda_release_high(soft_iic_obj);                            // SDA ???

    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_sda_drive_low(soft_iic_obj);                               // SDA ???
    soft_iic_delay(soft_iic_obj->delay);
...
static uint8 soft_iic_wait_ack (soft_iic_info_struct *soft_iic_obj)
{
    uint8 temp = 0;
    soft_iic_gpio_low_scl();                                            // SCL ???
    soft_iic_sda_release_high(soft_iic_obj);                            // SDA ??? ?? SDA
    soft_iic_delay(soft_iic_obj->delay);
    ...
    soft_iic_gpio_low_scl();                                            // SCL ???
    soft_iic_sda_prepare_output(soft_iic_obj);                          // ??????
    soft_iic_delay(soft_iic_obj->delay);
...
static uint8 soft_iic_send_data (soft_iic_info_struct *soft_iic_obj, const uint8 data)
{
    ...
    while(temp)
    {
        if(data & temp)
        {
            soft_iic_sda_release_high(soft_iic_obj);
        }
        else
        {
            soft_iic_sda_drive_low(soft_iic_obj);
        }
        temp >>= 1;
...
{
    zf_assert(scl_pin != sda_pin);                                              // ??? scl_pin ? sda_pin ??????????
    ...
    gpio_init(scl_pin, GPO, GPIO_HIGH, GPO_PUSH_PULL);                          // ????IO?? AF????
#if SOFT_IIC_INTERNAL_PULLUP_COMPAT
    gpio_init(sda_pin, GPI, 0, GPI_PULL_UP);                                    // ?????????SDA??????
#else
    gpio_init(sda_pin, GPO, GPIO_HIGH, GPO_OPEN_DTAIN);                         // ????IO?? AF????
#endif
}
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     软件 IIC 地址探测（仅检测地址阶段ACK）
// 参数说明     *soft_iic_obj   软件 IIC 指定信息存放结构体的指针
// 参数说明     addr            7位器件地址
// 返回参数     uint8           1-检测到ACK 0-无ACK
//-------------------------------------------------------------------------------------------------------------------
uint8 soft_iic_probe_addr (soft_iic_info_struct *soft_iic_obj, uint8 addr)
{
    uint8 ack = 0;
    uint8 old_addr = soft_iic_obj->addr;

    soft_iic_obj->addr = addr;
    soft_iic_start(soft_iic_obj);
    ack = soft_iic_send_data(soft_iic_obj, (uint8)(soft_iic_obj->addr << 1));
    soft_iic_stop(soft_iic_obj);
