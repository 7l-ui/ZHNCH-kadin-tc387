...
static uint8 uart19_tx_buffer[1 + sizeof(Ifx_Fifo) + 8];
static uint8 uart19_rx_buffer[1 + sizeof(Ifx_Fifo) + 8];
//-------------------------------------------------------------------------------------------------------------------
// 函数简介       串口中断优先级设置
...
void uart_set_interrupt_priority (uart_index_enum uartn)
{
    switch(uartn)
    {
        ...
        case UART_19:
        {
            uart_config.interrupt.txPriority    = UART19_TX_INT_PRIO;
            uart_config.interrupt.rxPriority    = UART19_RX_INT_PRIO;
            uart_config.interrupt.erPriority    = UART19_ER_INT_PRIO;
            uart_config.interrupt.typeOfService = UART19_INT_SERVICE;
        }break;
...
void uart_set_buffer (uart_index_enum uartn)
{
    switch(uartn)
    {
        ...
        case UART_19:
        {
            uart_config.txBuffer     = &uart19_tx_buffer;
            uart_config.rxBuffer     = &uart19_rx_buffer;
            uart_config.txBufferSize = 1;
            uart_config.rxBufferSize = 1;
        }break;
        default: zf_assert(FALSE);
...
IfxAsclin_Asc* uart_get_handle (uart_index_enum uartn)
{
    ...
    switch(uartn)
    {
        ...
        case UART_19: uart_handle = &uart19_handle; break;
        default: zf_assert(FALSE);
    }
    return uart_handle;
}
...
void uart_mux (uart_index_enum uartn, uart_tx_pin_enum tx_pin, uart_rx_pin_enum rx_pin, uint32 *set_tx_pin, uint32 *set_rx_pin)
{
    switch(uartn)
    {
        ...
        case UART_19:
        {
            if     (UART19_TX_P15_4 == tx_pin)  *set_tx_pin = (uint32)&IfxAsclin19_TX_P15_4_OUT;
            else if(UART19_TX_P15_7 == tx_pin)  *set_tx_pin = (uint32)&IfxAsclin19_TX_P15_7_OUT;
            else zf_assert(FALSE);

            if     (UART19_RX_P15_6 == rx_pin)  *set_rx_pin = (uint32)&IfxAsclin19_RXA_P15_6_IN;
            else if(UART19_RX_P15_7 == rx_pin)  *set_rx_pin = (uint32)&IfxAsclin19_RXB_P15_7_IN;
            else zf_assert(FALSE);
